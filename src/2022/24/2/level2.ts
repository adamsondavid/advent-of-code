...
export default class extends Level {
  public run() {
    ...
    const start: Vec2 = { x: field[0].indexOf("."), y: 0 };
    const exit: Vec2 = { x: field[field.length - 1].indexOf("."), y: field.length - 1 };

    let currentDestination = exit;
      ...
      for (const player of players) {
        for (const direction of [{ x: 0, y: 0 }, UP, DOWN, LEFT, RIGHT]) {
          ...
          if (blizzards.some((blizzard) => blizzard.position.x === newPlayer.x && blizzard.position.y === newPlayer.y))
            continue;
          if (newPlayer.x === currentDestination.x && newPlayer.y === currentDestination.y) {
            if (way === 0) currentDestination = start;
            if (way === 1) currentDestination = exit;
            if (way === 2) return min;
            way++;
            players = [newPlayer];
            continue outer;
