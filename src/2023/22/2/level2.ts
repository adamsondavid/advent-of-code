...
export function solve(input: StringStream) {
  ...
  for (const cube of cubes) {
    ...
    for (const subCube of cube.subCubes) mapCopy[subCube.z][subCube.x][subCube.y] = false;
    for (const c of cubes) {
      if (c === cube) continue;
      for (const subCube of c.subCubes) mapCopy[subCube.z][subCube.x][subCube.y] = false;
      const dZ = deltaZ(c, mapCopy);
      for (const subCube of c.subCubes) mapCopy[subCube.z + dZ][subCube.x][subCube.y] = true;
      if (dZ !== 0) ans++;
    }
    for (const subCube of cube.subCubes) mapCopy[subCube.z][subCube.x][subCube.y] = true;
  }
  return ans;
}
